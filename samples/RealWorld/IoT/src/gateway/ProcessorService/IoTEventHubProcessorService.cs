...
namespace EventHubProcessor
{
    ...
    public class IoTEventHubProcessorService : StatefulService
    {
        ...
        public IoTEventHubProcessorService()
        {
            ...
            this.m_CompositeListener = new CompositeCommunicationListener(this.TraceWriter);


            this.m_CompositeListener.OnCreateListeningAddress = (listener, addresslist) =>
            {
        ...
        protected override ICommunicationListener CreateCommunicationListener()
        {
            ...
            */
            ProcessorServiceOwinListenerSpec spec = new ProcessorServiceOwinListenerSpec(); // in addition to standard Owin stuff  
            //this Specification class injects a service reference in every
            // API controller instance created
            spec.Svc = this;
            this.m_CompositeListener.AddListenerAsync(s_OwinListenerName, new OwinCommunicationListener(spec)).Wait();


            return this.m_CompositeListener;
        }
        ...
        protected override async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            this.m_EventHubListenerHandler = new EventHubListenerDataHandler(this.WorkManager);
        ...
        {
            return Task.Run(() => { return this.WorkManager.NumberOfActiveQueues; });
        }

        public Task<int> GetTotalPostedLastMinuteAsync()
        {
            return Task.Run(() => { return this.WorkManager.TotalPostedLastMinute; });
        }

        public Task<int> GetTotalProcessedLastMinuteAsync()
        {
            return Task.Run(() => { return this.WorkManager.TotalProcessedLastMinute; });
        }

        public Task<int> GetTotalPostedLastHourAsync()
        {
            return Task.Run(() => { return this.WorkManager.TotalPostedLastHour; });
        }

        public Task<int> GetTotalProcessedLastHourAsync()
        {
            return Task.Run(() => { return this.WorkManager.TotalProcessedLastHour; });
        }

        public Task<float> GetAveragePostedPerMinLastHourAsync()
        {
            return Task.Run(() => { return this.WorkManager.AveragePostedPerMinLastHour; });
        }

        public Task<float> GetAverageProcessedPerMinLastHourAsync()
        {
            return Task.Run(() => { return this.WorkManager.AverageProcessedPerMinLastHour; });
        }
        ...
        public bool IsInErrorState { // we maintain a seprate error flag because while worker might be in 
        ...
        public async Task SetAssignedProcessorAsync(Processor newProcessor)
        {
            ...
            this.m_AssignedProcessor = await this.SaveProcessorToState(newProcessor);
        ...
        private async Task ClearEventHubListeners()
        {
            ...
            foreach (KeyValuePair<string, ICommunicationListener> kvp in this.m_CompositeListener.Listners)
                if (kvp.Key != s_OwinListenerName)
                    await this.m_CompositeListener.RemoveListenerAsync(kvp.Key);
        }
        ...
        private async Task RefreshListenersAsync()
        {
            ...
            IReliableDictionary<string, string> LeaseStateDictionary =
                await this.StateManager.GetOrAddAsync<IReliableDictionary<string, string>>(s_def_dictionary);

            this.TraceWriter.TraceMessage(string.Format("Event Hub Leases are saved in reliable dictionary named {0}", s_def_dictionary));
            ...
            foreach (EventHubDefinition hub in processor.Hubs)
            {
                ...
                try
                {
                    EventHubCommunicationListener eventHubListener = new EventHubCommunicationListener(
                        ...
                        this.m_EventHubListenerHandler,
                    ...
                    await this.m_CompositeListener.AddListenerAsync(ListenerName, eventHubListener);
                }
                ...
                finally
                {
                    if (BadListener)
                    {
                        try
                        {
                            await this.m_CompositeListener.RemoveListenerAsync(ListenerName);
                        }
                        catch
                        {
                            /* no op*/
        ...
        private async Task<Processor> GetAssignedProcessorFromState()
        {
            ...
            IReliableDictionary<string, string> dict = await this.StateManager.GetOrAddAsync<IReliableDictionary<string, string>>(s_def_dictionary);

            using (ITransaction tx = this.StateManager.CreateTransaction())
            {
                ConditionalResult<string> cResult = await dict.TryGetValueAsync(tx, s_AssignedProcessorEntryName);
                if (cResult.HasValue)
        ...
        {
            // do we have it?
            if (null != this.m_AssignedProcessor)
                return this.m_AssignedProcessor;

            //is it in state
            Processor processor = await this.GetAssignedProcessorFromState();
            if (processor != null)
            {
                this.m_AssignedProcessor = processor;
                return this.m_AssignedProcessor;
            }
    ...
    // project 
            ...
            if (null != this.ServiceInitializationParameters.InitializationData)
            {
                ...
                this.m_AssignedProcessor = await this.SaveProcessorToState(initProcessor); // this sets m_assignedprocessor
